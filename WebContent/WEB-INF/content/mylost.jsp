		...
		<div id="templatemo_main_wrapper">
			<div id="templatemo_main">
				...
				<div id="content" class="right">
					<h2>我发布的信息</h2>
					<a class="button left" href="addlost">添加记录</a><br><br>
					<c:forEach items="${myllt}" var="lost">
				
						<div class="product_box">
							<a
								href="mylostdetail?lostid=${lost.getLostid() }"><img
							...
							<p class="add_to_cart">
								<a
									href="mylostdetail?lostid=${lost.getLostid()}">详情</a>
								<a
									href="detelemylost?lostid=${lost.getLostid()}">删除</a>											
							</p>
						</div>
					</c:forEach>
<br><br><br><br><br><br><br><br><br><br><br><br><br><br>
	<div class="blank_box">
		...
		<a class="button left" href="mylost?pageNow=1">首页</a>
		<c:choose>
			
			<c:when test="${page.pageNow - 1 > 0}">
				<a class="button left" href="mylost?pageNow=${page.pageNow - 1}">上一页</a>
			</c:when>
			<c:when test="${page.pageNow - 1 <= 0}">
				<a class="button left" href="mylost?pageNow=1">上一页</a>
			</c:when>
		</c:choose>
		...
		<c:choose>
			<c:when test="${page.totalPageCount==0}">
				<a class="button right" href="mylost?pageNow=${page.pageNow}">尾页</a>
			</c:when>
			<c:otherwise>
				<a class="button right" href="mylost?pageNow=${page.totalPageCount}">尾页</a>
			</c:otherwise>
		</c:choose>
			<c:choose>
			<c:when test="${page.totalPageCount==0}">
				<a class="button right" href="mylost?pageNow=${page.pageNow}">下一页</a>
			</c:when>
			<c:when test="${page.pageNow + 1 < page.totalPageCount}">
				<a class="button right" href="mylost?pageNow=${page.pageNow + 1}">下一页</a>
			</c:when>
			<c:when test="${page.pageNow + 1 >= page.totalPageCount}">
				<a class="button right" href="mylost?pageNow=${page.totalPageCount}">下一页</a>
			</c:when>
		</c:choose>
